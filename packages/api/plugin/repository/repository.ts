...
// Re-export Repository so generated code can import it from this module.
export type { Repository } from 'o18k-ts-aws';
...
export class DynamoDbRepository implements Repository {
	...
	public async load(observation: Observation2<IEntityObservation>, previousObservations: Query[]): Promise<Observation2<IEntityObservation>[][]> {
		let queryParams: any;
		const observationLists = new Array<Array<Observation2<IEntityObservation>>>();
		...
		for (let i = 0; i < previousObservations.length; i++) {
			...
			if (previousObservations[i].filter === filters.TOP_1_WHERE_ENTITY_EQUALS_AND_TYPE_EQUALS_AND_ENTITYID_EQUALS_VALUE) {
				const GSI1PK = `ET#${previousObservations[i].filterValues[0]}#TY#${previousObservations[i].filterValues[1]}#EI#${previousObservations[i].filterValues[2]}`;
				...
				const GSI1PK = `ET#${previousObservations[i].filterValues[0]}#TY#${previousObservations[i].filterValues[1]}#EI#${observation.entityid}`;
				...
				const observationFilterValues = previousObservations[i].filterValues;
				...
				const GSI1PK = `ET#${previousObservations[i].filterValues[0]}#TY#${previousObservations[i].filterValues[1]}#EI#${observation.data[observationDataProperty]}`;
				...
				const GSI2PK = `ET#${entity}#TY#${type}#D2#${D2}`;
	...
	public async save(observations: Observation2<IEntityObservation>[]) {
		...
		const responses = await Promise.all(batchWritePromises);
