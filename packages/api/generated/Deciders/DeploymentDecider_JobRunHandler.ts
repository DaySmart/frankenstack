    ...
    const deployment: Deployment.DataSchema = {
        ...
        Components: latestDeployment.data.Components.map(component => {
            if(component.Name === data.ComponentName) {
                let componentStatus: Deployment.ComponentDeploymentStatus = data.Error ? 'DEPLOYMENT_FAILED' : 'DEPLOY_IN_PROGRESS';
                let statusReason: Array<string> = component.StatusReason || [];
                if(data.Error) {
